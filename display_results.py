...
def get_profile_url(profile_id):
    profile_url = (
        'https://steamcommunity.com/profiles/' + str(profile_id) + '/inventory/#753'
    )
    return profile_url
...
def get_offer_markdown(profile_id, trade_offers=None):
    ...
    if partner is not None and token is not None:
        trade_offer_url = get_trade_offer_url(partner=partner, token=token)
...
def display_results_with_markdown(
    results,
